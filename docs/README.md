...
- VNIBB persistence is a single self-hosted database stack: market corpus, durable app/runtime state, auth, and cache. It is privately hosted and reachable from the backend over a private network, and is portable to other infrastructure in the future.
