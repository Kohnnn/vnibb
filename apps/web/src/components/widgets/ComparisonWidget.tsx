...
import { X, Search, LineChart } from 'lucide-react';
...
export function ComparisonWidget({ id, initialSymbols = ['VNM', 'FPT'], hideHeader, onRemove }: ComparisonWidgetProps) {
    ...
    const {
        ...
        isFetching,
        dataUpdatedAt,
    } = useComparison(symbols, { enabled: symbols.length >= 2 });
    ...
    useEffect(() => {
        ...
        const fetchPerf = async () => {
            setPerfLoading(true);
            try {
                const res = await fetch(`${API_BASE_URL}/comparison/performance?symbols=${symbols.join(',')}&days=30`);
                if (res.ok) {
    ...
    const metricKey = (metric: any) => metric?.id ?? metric?.key;
    const metricLabel = (metric: any) => metric?.name ?? metric?.label ?? metricKey(metric);
    ...
    const categoryMetrics = useMemo(() => {
        ...
        return data.metrics.filter((m: any) => keys.includes(metricKey(m)));
    }, [data, activeCategory]);

    const hasData = Boolean(data?.metrics?.length);
    ...
    return (
        ...
        >
            <div className="h-full flex flex-col bg-secondary overflow-hidden">
                ...
                {symbols.length < 2 ? (
                    <WidgetEmpty
                        message="Comparison needs at least two symbols."
                        action={{ label: 'Add FPT', onClick: () => setSymbols(Array.from(new Set([...symbols, 'FPT']))) }}
                    />
                ) : isLoading && !hasData ? (
