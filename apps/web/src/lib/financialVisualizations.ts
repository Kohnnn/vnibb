...
export function buildIncomeSankeyModel(rows: IncomeStatementData[]): IncomeSankeyModel | null {
  ...
  const links: FinancialFlowLink[] = [
    ...
    { source: 'operating_income', target: 'tax_expense', value: taxExpense, tone: '#fcd34d' },
    { source: 'operating_income', target: 'net_income', value: netIncome, tone: '#6ee7b7' },
...
export function buildCashFlowWaterfallModel(rows: CashFlowData[]): CashFlowWaterfallModel | null {
  ...
  return {
    ...
    maxValue,
    summary: {
      freeCashFlow: firstFinite(latest.free_cash_flow, null),
      capex: firstFinite(latest.capex, latest.capital_expenditure, null),
      dividends: firstFinite(latest.dividends_paid, null),
      netChange,
    },
  };
}
