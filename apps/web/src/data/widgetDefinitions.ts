...
const widgetDefaultLayoutOverrides: Partial<Record<WidgetType, WidgetDefinition['defaultLayout']>> = {
    ...
    market_overview: { w: 8, h: 9, minW: 5, minH: 5 },
    market_breadth: { w: 8, h: 10, minW: 6, minH: 7 },
    top_movers: { w: 8, h: 9, minW: 6, minH: 6 },
    peer_comparison: { w: 14, h: 8, minW: 10, minH: 6 },
...
widgetDefinitions.forEach((widget) => {
    const override = widgetDefaultLayoutOverrides[widget.type];
    if (override) {
        widget.defaultLayout = override;
    }
});
