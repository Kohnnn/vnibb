...
ALL_TABLES = [
    ...
    "companies", "shareholders", "officers", "subsidiaries",
    ...
    "block_trades", "insider_alerts", "alert_settings"
]
        ...
        try:
            ...
            if latest:
                if isinstance(latest, str):
                    try: latest = datetime.fromisoformat(latest.replace("Z", "+00:00"))
                    except: pass
                if isinstance(latest, datetime):
                    age = datetime.utcnow() - latest.replace(tzinfo=None)
                    if age.total_seconds() < 3600: freshness = "fresh"
                    elif age.total_seconds() < 86400: freshness = "recent"
            
            tables_info.append({
                "name": table_name,
                "count": count,
                "last_updated": latest.isoformat() if hasattr(latest, 'isoformat') else str(latest) if latest else None,
                "freshness": freshness,
            })
...
async def get_table_sample(
    ...
    try:
        ...
        if search:
            ...
            text_cols = [c["name"] for c in schema.get("columns", []) if "TEXT" in str(c["type"]).upper() or "CHAR" in str(c["type"]).upper()]
            if text_cols:
        ...
        for row in rows:
            ...
            for k, v in rd.items():
                ...
                elif isinstance(v, bytes): rd[k] = v.hex()
            data.append(rd)
            
        return {"table": table_name, "rows": data, "total": total, "count": len(data), "has_more": offset + len(data) < total}
    except Exception as e: return {"error": str(e)}
...
async def execute_query(query: str = Body(..., embed=True), db: AsyncSession = Depends(get_db)):
    ...
    if not query.strip().upper().startswith("SELECT"): raise HTTPException(status_code=400, detail="Only SELECT allowed")
    try:
...
async def get_sync_status(db: AsyncSession = Depends(get_db)):
    ...
    try:
        result = await db.execute(text("SELECT * FROM sync_status ORDER BY completed_at DESC LIMIT 50"))
        rows = result.mappings().all()
        jobs = []
        for row in rows:
            rd = dict(row)
    ...
    for table, date_col in tables_to_check:
        try:
            res = await db.execute(text(f"SELECT MAX({date_col}) as last_update, COUNT(*) as count FROM {table}"))
...
async def trigger_seed(seed_type: str, background_tasks: BackgroundTasks):
    ...
    return {"status": "started", "seed_type": seed_type, "message": "Seeding task started in background"}
