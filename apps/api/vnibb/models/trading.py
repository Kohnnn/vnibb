...
class IntradayTrade(Base):
    ...
    def __repr__(self) -> str:
        return f"<IntradayTrade(symbol='{self.symbol}', time='{self.trade_time}', price={self.price})>"
...
class OrderbookSnapshot(Base):
    ...
    __table_args__ = (
        Index("ix_orderbook_symbol_time", "symbol", "snapshot_time"),
    )
...
class FinancialRatio(Base):
    ...
    roic: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # Return on Invested Capital
    ...
    def __repr__(self) -> str:
        return f"<FinancialRatio(symbol='{self.symbol}', period='{self.period}', pe={self.pe_ratio})>"
