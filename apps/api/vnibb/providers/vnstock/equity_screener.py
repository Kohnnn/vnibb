...
def _is_rate_limit_error(error: Exception) -> bool:
    ...
    return (
        "quá nhiều" in error_msg or  # Vietnamese message
...
class StockScreenerParams(BaseModel):
    ...
    source: str = Field(
        default="KBS",
        description="Data source: KBS (default v3.4.0+), VCI, TCBS, DNSE, or vnstock (internal)",
    )
...
class ScreenerData(BaseModel):
    ...
    industry_name: Optional[str] = Field(None, alias="industryName", description="Industry classification")
    ...
    shares_outstanding: Optional[float] = Field(None, alias="sharesOutstanding", description="Shares outstanding (millions)")
    ...
    ev_ebitda: Optional[float] = Field(None, alias="valueBeforeEbitda", description="EV/EBITDA ratio")
    ebitda_on_stock: Optional[float] = Field(None, alias="ebitdaOnStock", description="EBITDA per share")
    ...
    gross_margin: Optional[float] = Field(None, alias="grossProfitMargin", description="Gross Profit Margin (%)")
    net_margin: Optional[float] = Field(None, alias="postTaxMargin", description="Net Profit Margin (%)")
    operating_margin: Optional[float] = Field(None, alias="operatingProfitMargin", description="Operating Profit Margin (%)")
    ebit_on_revenue: Optional[float] = Field(None, alias="ebitOnRevenue", description="EBIT/Revenue (%)")
    pre_tax_on_ebit: Optional[float] = Field(None, alias="preTaxOnEbit", description="Pre-Tax Profit/EBIT (%)")
    post_tax_on_pre_tax: Optional[float] = Field(None, alias="postTaxOnPreTax", description="Post-Tax/Pre-Tax (%)")
    
    # ==========================================================================
    # PER SHARE DATA
    # ==========================================================================
    eps: Optional[float] = Field(None, alias="earningPerShare", description="Earnings per Share (VND)")
    bvps: Optional[float] = Field(None, alias="bookValuePerShare", description="Book Value per Share (VND)")
    eps_change: Optional[float] = Field(None, alias="epsChange", description="EPS YoY Change (%)")
    bvps_change: Optional[float] = Field(None, alias="bookValuePerShareChange", description="BVPS YoY Change (%)")
    ebitda_on_stock_change: Optional[float] = Field(None, alias="ebitdaOnStockChange", description="EBITDA/Share YoY Change (%)")
    
    # ==========================================================================
    # GROWTH METRICS
    # ==========================================================================
    revenue_growth: Optional[float] = Field(None, alias="revenueGrowth", description="Revenue Growth YoY (%)")
    earnings_growth: Optional[float] = Field(None, alias="earningsGrowth", description="Earnings Growth YoY (%)")
    net_profit_growth: Optional[float] = Field(None, alias="netProfitGrowth", description="Net Profit Growth YoY (%)")
    
    # ==========================================================================
    # DIVIDEND
    # ==========================================================================
    dividend_yield: Optional[float] = Field(None, alias="dividendYield", description="Dividend Yield (%)")
    
    # ==========================================================================
    # LIQUIDITY RATIOS
    # ==========================================================================
    current_ratio: Optional[float] = Field(None, alias="currentPayment", description="Current Ratio")
    quick_ratio: Optional[float] = Field(None, alias="quickPayment", description="Quick Ratio")
    days_receivable: Optional[float] = Field(None, alias="daysReceivable", description="Days Receivable")
    days_payable: Optional[float] = Field(None, alias="daysPayable", description="Days Payable")
    
    # ==========================================================================
    # CAPITAL STRUCTURE & DEBT METRICS
    # ==========================================================================
    debt_to_equity: Optional[float] = Field(None, alias="debtOnEquity", description="Debt-to-Equity ratio")
    debt_to_asset: Optional[float] = Field(None, alias="debtOnAsset", description="Debt-to-Asset ratio")
    debt_to_ebitda: Optional[float] = Field(None, alias="debtOnEbitda", description="Debt/EBITDA ratio")
    equity_on_total_asset: Optional[float] = Field(None, alias="equityOnTotalAsset", description="Equity/Total Assets (%)")
    equity_on_liability: Optional[float] = Field(None, alias="equityOnLiability", description="Equity/Liability ratio")
    asset_on_equity: Optional[float] = Field(None, alias="assetOnEquity", description="Asset/Equity ratio (leverage)")
    payable_on_equity: Optional[float] = Field(None, alias="payableOnEquity", description="Payable/Equity ratio")
    capital_balance: Optional[float] = Field(None, alias="capitalBalance", description="Capital balance")
    short_on_long_debt: Optional[float] = Field(None, alias="shortOnLongDebt", description="Short-term/Long-term Debt")
    
    # ==========================================================================
    # CASH METRICS
    # ==========================================================================
    cash_on_equity: Optional[float] = Field(None, alias="cashOnEquity", description="Cash/Equity ratio")
    cash_on_capitalize: Optional[float] = Field(None, alias="cashOnCapitalize", description="Cash/Capitalization ratio")
    
    # ==========================================================================
    # EFFICIENCY METRICS
    # ==========================================================================
    revenue_on_asset: Optional[float] = Field(None, alias="revenueOnAsset", description="Revenue/Asset ratio (asset turnover)")
    revenue_on_work_capital: Optional[float] = Field(None, alias="revenueOnWorkCapital", description="Revenue/Working Capital")
    capex_on_fixed_asset: Optional[float] = Field(None, alias="capexOnFixedAsset", description="CapEx/Fixed Asset ratio")
    
    # ==========================================================================
    # OWNERSHIP
    # ==========================================================================
    foreign_ownership: Optional[float] = Field(None, alias="foreignOwnership", description="Foreign Ownership (%)")
    state_ownership: Optional[float] = Field(None, alias="stateOwnership", description="State Ownership (%)")
        ...
        def _fetch_sync() -> List[dict[str, Any]]:
            ...
            try:
                ...
                if query["symbol"]:
                    ...
                    # Get ratio_summary which includes PE, PB, ROE, EPS, etc.
                    df = screener.company.ratio_summary()
                    if df is not None and not df.empty:
                        # Debug: Log actual columns for troubleshooting
                        logger.debug(f"ratio_summary columns for {query['symbol']}: {df.columns.tolist()}")
                        ...
                        logger.debug(
                            ...
                            f"ps={record.get('ps')}, ev_per_ebitda={record.get('ev_per_ebitda')}, "
                            f"roe={record.get('roe')}, de={record.get('de')}"
                        )
                ...
                logger.info("Fetching ratio_summary for %d symbols from %s (source: %s) in parallel", 
                           len(symbols), exchange or "ALL", source)
                ...
                def _fetch_single_stock(symbol: str) -> Optional[dict[str, Any]]:
                    ...
                    try:
                        ...
                        if "quá nhiều" in error_msg or "rate" in error_msg.lower():
                            ...
                            raise e
                        logger.debug("Failed to fetch ratio_summary for %s: %s", symbol, e)
                    return None
                ...
                try:
                    ...
                    logger.warning(f"Parallel fetch timed out after {batch_timeout}s")
